...
export function validatePoolRegistry(input: unknown): PoolRegistryValidationResult {
  ...
  input.forEach((entry, index) => {
    ...
    const requiredStringFields = ['id', 'chain', 'dex', 'kind', 'poolAddress', 'startBlock'] as const;
    ...
    if (pool.kind !== undefined && pool.kind !== 'UNISWAP_V3_STYLE' && pool.kind !== 'UNISWAP_V2_STYLE') {
    ...
    validateAddress(pool.poolAddress, `POOL_ADDRESS_INVALID:${context}:poolAddress`, entryErrors);
    ...
    if (pool.baseToken !== undefined && pool.quoteToken !== undefined && pool.baseToken === pool.quoteToken) {
...
function validateToken(token: unknown, context: string, errors: string[]): void {
  if (!isRecord(token)) {
  ...
  validateAddress(token.address, `POOL_TOKEN_ADDRESS_INVALID:${context}`, errors);
...
function validateAddress(value: unknown, errorCode: string, errors: string[]): void {
