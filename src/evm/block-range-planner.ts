...
export function planBlockRanges(fromBlock: bigint, toBlock: bigint, chunkSize: bigint): BlockRange[] {
  if (fromBlock < 0n) {
    throw new Error(`BLOCK_RANGE_FROM_NEGATIVE:${fromBlock.toString()}`);
  }
  if (toBlock < fromBlock) {
    throw new Error(`BLOCK_RANGE_INVALID:${fromBlock.toString()}:${toBlock.toString()}`);
  }
  if (chunkSize <= 0n) {
