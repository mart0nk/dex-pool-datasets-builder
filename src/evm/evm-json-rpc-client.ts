...
export function createEvmJsonRpcClient(input: {
  ...
  async function request<T>(method: string, params: unknown[]): Promise<T> {
    ...
    if (json.error !== undefined) {
      throw new Error(`EVM_RPC_ERROR:${method}:${json.error.code ?? 'unknown'}:${json.error.message ?? ''}`);
    }
  ...
  return {
    getLogs(getLogsInput) {
      return request<EvmLog[]>('eth_getLogs', [{
        ...
        ...(getLogsInput.topics !== undefined ? { topics: getLogsInput.topics } : {}),
      }]);
    },
    getBlockByNumber(blockNumber) {
      return request<EvmBlock>('eth_getBlockByNumber', [toQuantityHex(blockNumber), false]);
    },
  };
}
