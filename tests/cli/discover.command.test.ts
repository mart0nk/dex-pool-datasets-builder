...
vi.mock('../../src/discovery/uniswap-v3-subgraph-discovery.js', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../../src/discovery/uniswap-v3-subgraph-discovery.js')>();
...
describe('discover command', () => {
  it('prints human-ranked output', async () => {
    await expect(runDiscoverCommand({ chain: 'base', top: '10' })).rejects.toThrow('process.exit(0)');
  ...
  it('outputs valid JSON', async () => {
    await expect(runDiscoverCommand({ chain: 'base', json: true })).rejects.toThrow('process.exit(0)');
  ...
  it('fails with explicit missing subgraph URL env error', async () => {
    ...
    await expect(runDiscoverCommand({ chain: 'base' })).rejects.toThrow('process.exit(1)');
